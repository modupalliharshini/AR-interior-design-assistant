...
import { X, RotateCw, Move, ZoomIn, ZoomOut, Camera, CheckCircle } from 'lucide-react';
...
const modelLoadedRef = useRef<boolean>(false);
  ...
  const [isPlaced, setIsPlaced] = useState(false); // ✅ controls when UI hides
  ...
  const loadAndPlace3DModel = async () => {
    ...
    try {
      ...
      model.position.y = (floorPlane?.position.y ?? -1.5) + (size.y * scaleFactor) / 2;
  ...
  const rotateObject = () => placedObject && (placedObject.rotation.y += Math.PI / 4);
  const scaleObject = (f: number) => placedObject && placedObject.scale.setScalar(placedObject.scale.x * f);
  ...
  return (
    <div className="fixed inset-0 bg-black z-50">
      ...
      {/* ✅ Show controls only until placed */}
      {hasFloorDetected && placedObject && !isPlaced && (
        <div className="absolute bottom-6 left-0 right-0 flex flex-col items-center space-y-4 text-white">
          <div className="flex space-x-3">
            <button onClick={rotateObject} className="bg-white/20 p-4 rounded-full"><RotateCw /></button>
            <button onClick={() => scaleObject(1.2)} className="bg-white/20 p-4 rounded-full"><ZoomIn /></button>
            <button onClick={() => scaleObject(0.8)} className="bg-white/20 p-4 rounded-full"><ZoomOut /></button>
            <button className="bg-white/20 p-4 rounded-full"><Move /></button>
          </div>
      ...
      {/* Close button */}
      <button onClick={onClose} className="absolute top-4 right-4 bg-white/20 text-white p-3 rounded-full">
        <X />
      </button>
    </div>
  );
